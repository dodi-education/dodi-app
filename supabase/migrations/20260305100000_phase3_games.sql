-- Phase 3: Games system (system + custom + sandbox metadata)
...
create table public.games (
  ...
  subject text not null default 'creativity',
  difficulty text not null default 'easy',
  target_age_min int not null default 4,
  target_age_max int not null default 12,
  ...
  constraint games_system_row_check check (
    (
      is_system = true
  ...
  constraint games_duration_check check (estimated_duration_minutes between 1 and 180),
  constraint games_created_by_check check (created_by in ('system', 'ai', 'kid'))
);

-- reverse: drop table public.games;

create index games_account_profile_created_idx
  on public.games(account_id, profile_id, created_at desc);
create index games_system_created_idx
  on public.games(is_system, created_at desc);
create index games_tags_gin_idx
...
create policy "Users can view own and system games"
  on public.games for select
  using (is_system = true or auth.uid() = account_id);

create policy "Users can create own custom games"
  on public.games for insert
  with check (auth.uid() = account_id and is_system = false);

create policy "Users can update own custom games"
  on public.games for update
  using (auth.uid() = account_id and is_system = false)
  with check (auth.uid() = account_id and is_system = false);

create policy "Users can delete own custom games"
...
values (
  ...
  true,
  'Drawing Playground',
  'A simple drawing game with colors, brush sizes, and fun Dodi drawing commands.',
  'creativity',
  'easy',
  3,
  12,
  15,
  array['drawing', 'art', 'creativity', 'avatar'],
  $drawing$
<!doctype html>
<html lang="en">
<head>
