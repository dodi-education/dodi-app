-- Add draw_curve command (Bezier curves) to the Drawing game
-- reverse: Re-run 20260310100000 + 20260312100000 to restore previous version
UPDATE public.games
SET
code_bundle = $code$
<!doctype html>
...
<body>
  ...
  <script>
    (function () {
      ...
      const capabilities = ['set_color', 'set_brush_size', 'draw_shape', 'draw_line', 'draw_curve', 'clear_canvas', 'undo'];
      ...
      function handleCommand(command) {
        ...
        switch (type) {
          ...
          case 'draw_curve': {
            const x1 = Number(payload.x1), y1 = Number(payload.y1);
            const x2 = Number(payload.x2), y2 = Number(payload.y2);
            const cx1 = Number(payload.cx1), cy1 = Number(payload.cy1);
            if ([x1, y1, x2, y2, cx1, cy1].some(function (n) { return Number.isNaN(n); })) {
              return { ok: false, error: 'Invalid coordinates' };
            }
            const cx2 = payload.cx2 != null ? Number(payload.cx2) : null;
            const cy2 = payload.cy2 != null ? Number(payload.cy2) : null;
...
metadata = '{
    "version": "1.2.0",
    "capabilities": ["set_color", "set_brush_size", "draw_shape", "draw_line", "draw_curve", "clear_canvas", "undo"],
    "category": "drawing",
