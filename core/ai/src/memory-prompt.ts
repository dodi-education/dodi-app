...
2. A transcript of one or more recent sessions. Each session is headed by \`### Session <ISO timestamp>\` and every line is prefixed with its own ISO timestamp.

Your task: produce an UPDATED memory document that merges new observations from the session(s) into the existing memory.
...
- Take the date from the session transcript's ISO timestamps (the day the conversation actually happened) — NOT today's date, since memory may be processed on a later day.
