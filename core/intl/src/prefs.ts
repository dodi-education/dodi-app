 ...
 * chain profile → account → context default, so a kid can override only the
 * timezone while inheriting the rest.
 *
 * Context-aware defaults: the parent/account area defaults to the compact
 * numeric form, while kid profiles default to the more readable long form.
...
export type FormatContext = "account" | "profile";

/**
 * On-disk shape of the `date_preferences` jsonb column on `accounts`/`profiles`.
...
export function defaultPref(locale: string, context: FormatContext): DateFormatPref {
  return {
    timeZone: "auto",
    dateStyle: context === "profile" ? "long" : "numeric",
    timeStyle: isGerman(locale) ? "24h" : "12h",
  };
}

/**
 * Merge profile over account over the context default, per field. Absent fields
...
export function resolvePref(
  ...
  profile?: PartialDateFormatPref | null,
): DateFormatPref {
  const base = defaultPref(locale, context);
  return {
    timeZone: profile?.timeZone ?? account?.timeZone ?? base.timeZone,
    dateStyle: profile?.dateStyle ?? account?.dateStyle ?? base.dateStyle,
    timeStyle: profile?.timeStyle ?? account?.timeStyle ?? base.timeStyle,
  };
}
