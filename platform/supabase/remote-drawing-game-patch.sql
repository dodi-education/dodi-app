...
--  1) AI image-generation flow (generate_drawing -> set_generated_image).
--  2) Standardized toolbox command names (set_color -> set_drawing_color); the
--     voice-facing vocabulary is declared in metadata.capabilities.
--  3) Canvas-sizing race fix: a ResizeObserver re-sizes the drawing buffer when
...
',
  metadata = '{"version": "2.1.0", "category": "drawing", "capabilities": ["generate_drawing", "set_drawing_color", "set_brush_size", "clear_canvas", "undo", "get_snapshot"], "supportsVoiceCommands": true}',
  markdown = '# Drawing
...
- First emit the tool call, THEN speak a very short acknowledgement in the language of the child e.g. "Here it is!"
