-- Idempotent patch: set the drawing color picker to black, white, grey, brown (#894b17),
-- then red/orange/yellow/green/blue/purple/pink for the Drawing (drawing-basic) and
-- Mandala (mandala-basic) system games. Supersedes any earlier palette patch.
--
-- seed.sql defines these but a db reset/seed does NOT re-apply to the linked remote, so this
-- brings an already-initialized remote up to date. Full-column replace of the Drawing code_bundle
-- (never anchor-replace code_bundle on remote); Mandala shares the Drawing bundle with only <title>
-- swapped, so it is re-derived. Markdown allowed-color lists refreshed for both. Safe to re-run.
BEGIN;
...
UPDATE public.games SET
  code_bundle = replace(
    (SELECT code_bundle FROM public.games WHERE system_key = 'drawing-basic'),
...
- Mandala should be easy to color for kids (bigger grid size).
